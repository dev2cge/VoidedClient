...
public final class VoidedClientForge {
    ...
    private static void castIfPressed(KeyMapping mapping, String skill) {
        while (mapping.consumeClick()) {
            ...
            if ("dash".equals(skill)) try {
                Minecraft mc = Minecraft.getInstance();
                if (mc.player != null) {
                    net.minecraft.world.phys.Vec3 velocity = mc.player.getDeltaMovement();
    ...
    private static void sendHello() {
        send(VoidedProtocol.hello("forge", "1.7.1", SharedConstants.getCurrentVersion().getName(), VoidedProtocol.CAP_RPG_SKILLS));
    }
